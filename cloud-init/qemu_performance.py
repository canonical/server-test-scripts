...
MANDATORY_REPORT_KEYS = ("time_systemd_userspace", "client_time_to_ssh")
...
def update_cloud_init_in_img(img_path: str, deb_path: str, suffix=".modified"):
    ...
    if deb_path.endswith(".deb"):
        commands = [
            ...
            #         "sh",
            #         "-c",
            #         f"chroot ${{MOUNTPOINT}} dpkg -i /{os.path.basename(deb_path)}",
            #     ],
        ]
        ...
        commands = [
            [
                ...
                "sh",
                "-c",
                f"add-apt-repository ppa:cloud-init-dev/daily -y",
            ],
            [
                "sudo",
                "mount-image-callback",
                ...
                "sh",
                "-c",
                f"DEBIAN_FRONTEND=noninteractive apt-get install cloud-init -y",
            ],
        ]
    else:
        raise RuntimeError(
...
def collect_bootspeed_data(instance):
    ...
    data["journalctl"] = instance.execute("journalctl -o short-precise")
    return data
...
def process_bootspeed_data(boot_sample: dict):
    ...
    analyze_times = re.match(
        r".*in (?P<kernel>[^s]+)s \(kernel\) \+ (?P<user>[^s]+)s \(userspace\)",
...
METRIC_LINE = """| {avg:06.2f} | {max:06.2f} | {min:06.2f} | {name}"""


def print_avg_table(image_name, avg):
    print(METRIC_TABLE_HEADER.format(image_name=image_name))
    for key in avg:
        if "blame" in key:
...
| Control Avg | Upgrade Avg | Avg delta | Delta type and service name
-----------------------------------------------------------------------------"""

PERF_LINE = """\
|      {orig_avg:06.2f} |      {new_avg:06.2f} |   {avg_delta:+06.2f}s | ***{delta_type} {name:<24}"""


def report_significant_avg_delta(key, orig_sample, new_sample):
    """Return string describing service boottime deltas when significant.
    ...
    :return: tabular formatted string declaring significantly IMPROVED or
...
def inspect_averages(orig_avg, new_avg):
    ...
    print_avg_table("Control", orig_avg)
    print_avg_table("Updated cloud-init", new_avg)
...
def inspect_boot_time_deltas_for_deb(
    ...
    if existing_data_files:
        print("--- Using pre-existing data files in {data_dir}")
        ...
        with pycloudlib.Qemu(tag="examples") as cloud:
            ...
            print(f"--- Launching {BOOT_SAMPLES} control daily images {daily} ---")
...
def get_parser():
    ...
    parser.add_argument(
        ...
        "--release",
        default="noble",
        choices=["focal", "jammy", "lunar", "mantic", "noble"],
